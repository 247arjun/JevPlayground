...
- A changed snapshot gets a new generation; semantic results are keyed by snapshot/configuration/compiler identity. Fine-grained cross-snapshot dependency invalidation/reuse is not yet implemented, so a refreshed dataset uses a new run.
