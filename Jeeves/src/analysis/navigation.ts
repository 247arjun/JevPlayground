...
export class Navigation {
  ...
  constructor (readonly store: Store) { this.workspaces = new Workspaces(path.join(store.root, 'snapshot/source')) }
  async resolve (file: string, start: number, project: string): Promise<unknown> {
    ...
    const result = await this.workspaces.resolve(project, file, start)
  ...
  close (): void { this.workspaces.close() }
}